  ...
  return (
    <section className="w-full max-w-5xl mx-auto px-6 mt-40 relative z-10 text-left">
      <SectionHeading
        ...
        subtitle="Answers to common questions about Derivo's architecture and local verification capabilities."
      />
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full px-2 py-6 flex items-center justify-between text-left transition-colors hover:text-white focus:outline-none focus-visible:ring-1 focus-visible:ring-white rounded-full group"
                  aria-expanded={isOpen}
                  aria-controls={`faq-answer-${idx}`}
                  id={`faq-btn-${idx}`}
                >
                  <span className="font-medium text-white/80 group-hover:text-white text-sm transition-colors">
                    {faq.question}
                  </span>
                  <ChevronDown
                    className={cn(
                      'w-4 h-4 text-white/40 transition-transform duration-300 shrink-0 ml-4',
                      isOpen && 'rotate-180',
                    )}
                  />
                </button>
                ...
                >
                  <div className="px-2 pb-6 text-sm text-white/50 leading-relaxed font-light">
                    {faq.answer}
                  </div>
                </motion.div>
              </motion.div>
