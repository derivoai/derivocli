...
export function CLIDemo() {
  ...
  return (
    ...
    >
      {/* Glow halo behind the terminal */}
      <div
        aria-hidden
        className="absolute -inset-x-8 -top-12 -bottom-12 pointer-events-none animate-glow-breathe"
        style={{
          background:
            'radial-gradient(ellipse 60% 50% at 50% 45%, rgba(99,102,241,0.16) 0%, rgba(139,92,246,0.07) 45%, transparent 75%)',
          filter: 'blur(50px)',
        }}
      />
      {/* Gradient hairline border wrapper */}
      <div className="relative rounded-[32px] p-px bg-gradient-to-b from-white/[0.18] via-white/[0.06] to-white/[0.02] shadow-[0_0_70px_-12px_rgba(99,102,241,0.3),0_30px_80px_-30px_rgba(0,0,0,0.8)]">
        <div className="relative rounded-[31px] bg-[#0a0a0a]/95 backdrop-blur-sm overflow-hidden flex flex-col h-[480px] md:h-[520px]">
          {/* Terminal Top Window Bar */}
          <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-white/[0.08] bg-[#0c0c0c]/80">
              ...
              {lines.map((line, idx) => {
                ...
                if (line.type === 'spinner') {
                  return (
                    ...
                    >
                      <span className="w-3 h-3 shrink-0 rounded-full border border-white/20 border-t-indigo-300/90 animate-spin" />
                      <span>{line.text}</span>
                    </motion.div>
                  );
                }
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <motion.span
                          ...
                          className="text-emerald-300/80 select-none"
                        >
                          ✓
                        </motion.span>
                        <span>{line.text}</span>
            ...
            <div className="flex flex-col gap-1.5 flex-1">
              ...
              <div className="h-1.5 w-full bg-white/[0.06] rounded-full overflow-hidden">
                <div
                  ...
                  className="relative h-full rounded-full bg-gradient-to-r from-indigo-400 via-violet-400 to-amber-300 transition-all duration-700 ease-out overflow-hidden"
                >
                  <div className="absolute inset-y-0 w-1/2 bg-gradient-to-r from-transparent via-white/50 to-transparent animate-progress-shine" />
                </div>
              </div>
            </div>
          </div>
