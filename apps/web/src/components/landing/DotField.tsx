...
const DotField = memo(
  ({
    ...
    useEffect(() => {
      ...
      // Restart the render loop after it has gone to sleep (idle) — called on
      // mouse move, resize, or when the tab becomes visible again.
      function wake() {
        if (paused || awake) return;
        awake = true;
        idleFrames = 0;
      ...
      function tick() {
        ...
        // Stop redrawing identical frames once the mouse is idle and the grid
        // has settled (no continuous wave/sparkle in this config). A mouse move,
        // resize, or tab-visible event re-wakes the loop via wake().
        const continuous = (p.waveAmplitude as number) > 0 || (p.sparkle as boolean);
        if (!continuous && eng < 0.002 && m.speed < 0.02) {
          idleFrames++;
        } else {
          idleFrames = 0;
        }
        if (!continuous && idleFrames > IDLE_LIMIT) {
          awake = false;
          return;
        }
        rafRef.current = requestAnimationFrame(tick);
      ...
      function onVisibilityChange() {
        ...
        if (!paused) {
          // Loop stopped scheduling while hidden/idle — kick it back on.
          awake = false;
          wake();
        }
      }
